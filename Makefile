...
typecheck: ## Run type checking with mypy
	uv run mypy aws_knowledge_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r aws_knowledge_tool -c pyproject.toml
...
run: ## Run aws-knowledge-tool (usage: make run ARGS="...")
...
uninstall-global: ## Uninstall global installation
	uv tool uninstall aws-knowledge-tool
